...
test.describe("Make Appointment", () => {
    test.beforeEach("Login with valid creds", async ({ page }, testInfo) => {
        ...
        // ADD custom screenshot at this point to capture
        let fullScreenshotPage = await page.screenshot({fullPage: true});
        await log("info", "Screenshot captured on failure");
        await testInfo.attach("fullPageScreenshot", {
            body: fullScreenshotPage,
    ...
    test("Should make an appointment with non-default values", async ({ page }, testInfo) => {
        ...
        // commenting line below will fail the test and capture a screenshot of the failure
